...
**개인키 기기에 백업하기**는 현재 등록된 모든 지갑의 개인키와 이름을 하나의 암호화된 JSON 파일에 저장합니다. 지갑을 추가한 뒤 다시 백업하면 기존 지갑과 새 지갑이 함께 포함됩니다. 기존 다운로드 파일은 자동 갱신되지 않습니다. 파일을 다시 열어 비밀번호로 검증해야 포함된 지갑을 사용할 수 있고, 백업하지 않은 지갑은 계속 사용이 제한됩니다. **백업된 개인키 불러오기**는 파일에 담긴 모든 지갑을 한 번에 복원하며, 기존 지갑은 유지하고 중복은 추가하지 않습니다. 이전 단일 지갑 백업도 불러올 수 있습니다. 기존 저장소가 잠겨 있으면 먼저 잠금을 해제해야 합니다.

사이트 데이터를 삭제해도 별도로 저장한 파일은 남지만, 기기 분실·고장에 대비해 최신 파일의 사본을 다른 안전한 장소에도 보관하세요. 새 지갑을 추가할 때마다 통합 백업을 갱신해야 하며, 새 기기로 파일을 옮겨 불러올 수 있습니다. 지갑 잠금 비밀번호 변경은 기존 백업 파일의 비밀번호를 변경하지 않습니다. 백업 비밀번호를 잊으면 파일로 복구할 수 없습니다.

**기존 백업 업데이트**에서 파일과 기존 백업 비밀번호를 선택하면 파일 속 지갑과 현재 지갑을 병합합니다. 파일에만 있던 지갑과 이름을 보존하고 개인키 중복을 제거합니다. 추가 지갑 수를 확인하고 동의하면, 파일 쓰기 API 지원 환경에서는 쓰기 권한을 받아 같은 파일을 갱신합니다. 저장 직전 원본 변경 여부와 저장 후 내용 일치를 확인해야 백업 완료로 처리합니다. 미지원 환경에서는 병합된 새 파일을 다운로드하고 다시 열어 검증합니다. 백업 파일이나 비밀번호는 서버로 전송하지 않습니다.
